...
def runpf(casedata=None, ppopt=None, fname='', solvedcase=''):
    ...
    if dc:                               # DC formulation
        if verbose:
            pass #stdout.write(' -- DC Power Flow\n')
            ...
            if qlim:             ## enforce generator Q limits
                ...
                if len(mx) > 0 or len(mn) > 0:  ## we have some Q limit violations
                    ...
                    if len(pv) == 0:
                        if verbose:
                            if len(mx) > 0:
                                pass #print('Gen %d [only one left] exceeds upper Q limit : INFEASIBLE PROBLEM\n' % mx + 1)

                            else:
                                pass #print('Gen %d [only one left] exceeds lower Q limit : INFEASIBLE PROBLEM\n' % mn + 1)
                    ...
                    if verbose and len(mx) > 0:
                        for i in range(len(mx)):
                            pass #print('Gen ' + str(mx[i] + 1) + ' at upper Q limit, converting to PQ bus\n')

                    if verbose and len(mn) > 0:
                        for i in range(len(mn)):
                            pass #print('Gen ' + str(mn[i] + 1) + ' at lower Q limit, converting to PQ bus\n')
                    ...
                    if verbose and ref != ref_temp:
                        pass #print('Bus %d is new slack bus\n' % ref)
    ...
    if fname:
        ...
        pass #printpf(results, stdout, ppopt)
